...
export abstract class BaseInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    const options = { ...new AuthConfig(), ...this.injector.get<AuthConfig>(AuthConfig, undefined) };
    if (options.ignores) {
    ...
    if (this.isAuth(options)) {
      ...
      const err$ = new Observable((observer: Observer<HttpEvent<any>>) => {
        const res = new HttpErrorResponse({
          ...
          headers: req.headers,
          status: 401,
          statusText: `来自 auth 的拦截，所请求URL未授权，若是登录API可加入 [url?_allow_anonymous=true] 来表示忽略校验，更多方法请参考： https://ng-alain.com/auth/getting-started#AuthConfig\nThe interception from auth, the requested URL is not authorized. If the login API can add [url?_allow_anonymous=true] to ignore the check, please refer to: https://ng-alain.com/auth/getting-started#AuthConfig`,
        });
        observer.error(res);
      });
